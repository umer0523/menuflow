...
- **Availability computed server-side, in one place.** An item is visible at location `L` iff `(present_at_all_locations OR L ∈ present_at_location_ids) AND L ∉ absent_at_location_ids` — `absent` always wins. The grouped menu, category list, and flat item list are all projections of the same builder, so the three endpoints can never disagree. Time-of-day / day-of-week availability uses Square's `CatalogAvailabilityPeriod` objects (which attach to **categories**), evaluated fresh per request (not inside the 60 s cached snapshot) via `isAvailableNow()` with `Intl.DateTimeFormat` in the location's IANA timezone — no date library required. Categories carry `available` + `availabilityWindows`; **items inherit** their category's availability, so `GET /items` and `GET /items/:id` also carry the flags. Categories/items with no periods are always available; out-of-window items get a muted "disabled" card and a readable "Available 7 AM–11 AM" badge (on each menu card and the item-detail page), and the closed category's schedule is announced to screen readers. Set the server log level to `debug` to see the raw availability data and per-request timezone/day/time resolution in the console.
...
> _Given another week, roughly in priority order:_ modifiers on the item detail view, full-text search across the visible menu, a cart with subtotal, inventory/out-of-stock surfacing, offline-friendly catalog caching.
