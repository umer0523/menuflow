...
- **Availability computed server-side, in one place.** An item is visible at location `L` iff `(present_at_all_locations OR L ∈ present_at_location_ids) AND L ∉ absent_at_location_ids` — `absent` always wins. The grouped menu, category list, and flat item list are all projections of the same builder, so the three endpoints can never disagree. Category time-of-day / day-of-week availability is evaluated fresh per request (not inside the 60 s cached snapshot) via `isAvailableNow()`, using `Intl.DateTimeFormat` in the location's IANA timezone — no date library required. Categories without availability periods are always visible; categories outside their window are hidden entirely from the menu and filter.
